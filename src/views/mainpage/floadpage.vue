...
<div class="main-content">
      <el-breadcrumb separator-class="el-icon-arrow-right" class="directory">
          ...
          <el-breadcrumb-item @click.native="backPath"  :disabled="isDisable">返回上一级</el-breadcrumb-item>
...
export default {
  ...
  computed:{
    ...
    content(){
      return this.$route.params.floadName;
    }
  ...
  mounted() {
   ...
   this.pagePath= "/"+this.$route.params.floadName;
    axios.get(`http://152.136.111.227:8083/showPathFile?path=${this.pagePath}`).then((res)=>{
        
        const data = res.data.data;
        
        this.pageFile=data;
    });
    axios.get(`http://152.136.111.227:8083/showPathFload?path=${this.pagePath}`).then((res)=>{
      const data = res.data.data;
  ...
  watch:{
      ...
      $route(to) {
      // 对路由变化作出响应...
        this.pagePath= "/"+to.params.floadName;
    axios.get(`http://152.136.111.227:8083/showPathFile?path=${this.pagePath}`).then((res)=>{
  ...
  methods: {
    ...
    filecreate() {
      ...
      fload.floadPath = this.pagePath;
      fload.show = true;
      fload.floadId = num;
      fload.canClick = false;
